...
const GAP_PCT := 12        # percent of border gate-cells that contain a doorway
const GATE_WIDTH := 130.0  # the doorway itself is TINY — one body wide
const GUARDS_PER_GATE := 2 # entrance guards posted in each doorway
...
func _make(c: Vector2i):
	...
	if absf(pos.length() - Config.COMMONS_RADIUS) < TILE * 1.8:
		for di in _spawn_doors.size():
			...
			if pos.distance_to(d) <= GATE_WIDTH:
	...
	if posmod(gh, 100) < GAP_PCT:
		...
		if pos.distance_to(doorway) <= GATE_WIDTH:
...
func _post_guards(pos: Vector2, biome: String) -> void:
	...
	for i in GUARDS_PER_GATE:
		var off := Vector2(randf_range(-50, 50), randf_range(-50, 50))
		var e := EnemyTypes.spawn(arch)
		e.setup(biome, player, game._hp_scale())
		e.guard = true
