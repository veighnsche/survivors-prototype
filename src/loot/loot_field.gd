...
extends Node2D
## Places floor loot ON the map: boosters and chests (each its own class in
## src/loot/). Gold is dropped by monsters, not placed here. Deterministically
## seeds each world cell, streams it in around the player, and remembers
## collected cells so loot never respawns.

# The pool a loot cell rolls from (order matters: it feeds a deterministic hash).
const BOOSTERS := [
	preload("res://src/loot/heal_pack.gd"),
	preload("res://src/loot/magnet_lure.gd"),
	preload("res://src/loot/bomb_charge.gd"),
	preload("res://src/loot/frenzy_charm.gd"),
	preload("res://src/loot/power_charm.gd"),
	preload("res://src/loot/haste_charm.gd"),
	preload("res://src/loot/barrier_charm.gd"),
]
...
func _make(c: Vector2i, cell: float):
	...
	var node
	if roll < 86:
		node = BOOSTERS[posmod(ht >> 17, BOOSTERS.size())].new()
