class_name Upgrades
extends RefCounted
## The Vital pool — character-level (body) upgrades. This file owns both the
## card definitions AND their effects. Family spells do NOT live here; those
## come from biome essence and use (Insight tiers). See DESIGN.md.
...
## Apply a Vital card to the caster. Returns false if the id isn't ours
## (family deepening cards are owned by their Family — see Family.apply_minor).
